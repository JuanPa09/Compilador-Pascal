...
namespace _OLC2__Proyecto1.interprete.instruccion
{
    class Objeto
    {
        ...
        public Objeto(string nombre, Dictionary<string,Simbolo> variables, Dictionary<string,Tipo> tipoVariable)
        {
            this.nombre = nombre;
        ...
        public void setValor(string propiedad, Simbolo valor)
        {
            if (!variables.ContainsKey(propiedad))
                throw new ErrorPascal(0,0,"El objeto \""+nombre+"\" no contiene la propiedad "+propiedad,"");

            if (tipoVariable[propiedad].tipo != valor.tipo.tipo && tipoVariable[propiedad].tipoAuxiliar != valor.tipo.tipoAuxiliar)
                throw new ErrorPascal(0,0,"No se puede asignar el valor \""+valor.valor+"\" a \""+nombre+"."+propiedad+"\" porque los datos no coinciden","semantico");
